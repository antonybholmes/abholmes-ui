...
const DAY_BUTTON_CLS = clns(CENTERED_BUTTON_CLS, PILL_BUTTON_CLS, SIZE, FOCUS_BORDER_CLS, PRIMARY_FOCUS_BORDER_CLS, "relative shrink-0 grow-0 border-2");
function MonthView({ date, now, dateRange, rangeMode, onDayClick, onDayHover, }) {
    ...
    return (_jsxs(BaseCol, { className: "gap-y-2", onMouseEnter: () => setHover(true), onMouseLeave: () => setHover(false), children: [_jsx(HCenterRow, { children: format(date, "LLLL") }), _jsxs("div", { className: "grid grid-cols-7 gap-y-1 ", style: { width: W }, children: [SHORT_DAYS.map((day, di) => (_jsx("div", { className: clns(CENTERED_BUTTON_CLS, SIZE, "text-xs font-semibold text-slate-400 dark:text-gray-400"), children: day }, di))), days.map((day, di) => {
                        ...
                        return (_jsxs("div", { className: clns("relative text-xs", SIZE, CENTERED_BUTTON_CLS, [highlighted, "bg-theme-100 dark:bg-gray-600 "], [di % 7 === 0 || day.date.getDate() === 1, "rounded-l-full"], [
...
export function DateRangePicker({ currentDateRange, onDatesChange, onClose, }) {
    ...
    return (_jsxs("div", { className: "p-2", children: [_jsxs("nav", { className: "flex flex-row items-center justify-center gap-x-1 font-semibold", onClick: () => setYearMode(false), children: [_jsx(ToolbarIconButton, { onClick: displayPrevMonth, "aria-label": "Previous month", children: _jsx(ChevronRightIcon, { className: clns(CHEVRON_CLS, "rotate-180"), size: "w-3" }) }), _jsx(ToolbarButton, { onClick: toggleYearSelector, "aria-label": "Change year", children: startMonthDate.getFullYear() }), _jsx(ToolbarIconButton, { onClick: displayNextMonth, "aria-label": "Next month", children: _jsx(ChevronRightIcon, { className: CHEVRON_CLS, size: "w-3" }) })] }), _jsxs("div", { className: "relative py-2", children: [_jsxs(BaseRow, { className: "gap-x-8", children: [_jsx(MonthView, { date: startMonthDate, dateRange: highlightDateRange, rangeMode: rangeMode, now: now, onDayClick: onStartDayClick, onDayHover: onDayHover }), _jsx(MonthView, { date: endMonthDate, dateRange: highlightDateRange, rangeMode: rangeMode, now: now, onDayClick: onStartDayClick, onDayHover: onDayHover })] }), _jsx("div", { className: "px-2 py-2", children: _jsx(BlueLinkButton, { onClick: (e) => onDateClick(new Date()), "aria-label": "Today", className: "text-sm", children: "Today" }) }), yearMode && (_jsx("ul", { ref: yearMenuRef, className: "bg-popover absolute left-0 top-0 flex h-full w-full flex-col overflow-y-auto font-semibold", children: years.map(year => (_jsx("li", { children: _jsx(BaseButton, { id: `year-${year}`, className: clns(MENU_BUTTON_CLS, "shrink-0", [
