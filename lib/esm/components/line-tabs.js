...
const TAB_CLS = cn(BASE_BUTTON_CLS, FOCUS_RING_CLS, CENTERED_BUTTON_CLS, ROUNDED_BUTTON_CLS, "px-2 py-3.5 text-xs font-semibold");
...
export default function LineTabs({ tabs, activeTabIndex = 0, onTabClick, className, }) {
    ...
    return (_jsx(_Fragment, { children: _jsxs(BaseCol, { className: cn("gap-y-1", className), children: [_jsxs("div", { className: "relative", children: [_jsx(BaseRow, { className: "relative z-10 gap-x-1", children: tabs.map((tab, idx) => {
                                return (_jsx(BaseButton, { ref: el => (tabsRef.current[idx] = el), className: cn(TAB_CLS, [
                                        idx === activeTabIndex,
